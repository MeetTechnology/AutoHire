...
The system SHALL render the "Initial CV review extract" section as a semantic table in the unified CV Review experience, with each configured extract field shown as a row containing the field label and its display value.
...
- **THEN** the unified CV Review experience SHALL render a table with one row per configured field in the existing display order
