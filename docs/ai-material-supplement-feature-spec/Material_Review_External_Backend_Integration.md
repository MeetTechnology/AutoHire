...
- `src/lib/material-review/live.ts` — HTTP 路径、请求体、响应解析（Zod）
...
AutoHire **不会**在三条对外 HTTP 中发送简历全文、材料文件二进制或 OSS 直链；**仅**发送 `applicationId`（及 `category`）。你们需自行定义如何根据 `applicationId` 获取审查所需数据（共享数据源、内部同步表、或未来双方扩展的只读 API 等）。
...
| `MATERIAL_REVIEW_API_KEY` | 是 | 以 `Authorization: Bearer <值>` 发送 |
